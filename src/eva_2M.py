...
File to make validation of generated WSP based on model 2
'''

from plotprops import *
import os

def check_validation_eva2(building="B2E1",
                          apartments = ["A01", "A02", "A03", "A04", "A05", "A06", "A07", "A08", "A09", "A10"],
                          recFolder='D:/WinProGen/Data/',
                          years=2):
    recordFolder = 'D:/WinProGen/Data/validation/pictures/' + building + '_2M/'
    ...
    df2=pd.read_csv(recFolder+'validation/temp/M2_' + str(years) + '_'+ building + '_' + '_'.join(apartments) + '.csv', index_col=0, sep=';', header=[0,1,2,4,5,6,7],skiprows=[8],  parse_dates=True,low_memory=False)
    ...
    resultDF.to_csv(recordFolder+"results"+building+"_M2.csv", ';')
    resultDFAbs=DataFrame(resultsMeanAbsRes, index=indicis,columns=columns4Results)
    resultDFAbs.to_csv(recordFolder+"resultsMeanAbsRes"+building+"_M2.csv", ';')